...
def signup():
    ...
    try:
        ...
        return {
            "msg": "Senha deve conter ao menos 8 caracterers, uma letra maiúscula, uma minúscula, um número e um caractere especial"
        }, HTTPStatus.BAD_REQUEST
    except CPFError:
        return {
            "msg": "Formato do CPF inválido. Formatos válidos: 123.456.789-00 ou 12345678900"
        }, HTTPStatus.BAD_REQUEST
        ...
        return {
            "msg": "As chaves: nome, email, CPF e senha são obrigatórias"
...
def signin():
    ...
    return {
        "token": create_access_token(personal, expires_delta=timedelta(hours=1)),
        "user": personal,
    }, HTTPStatus.OK
...
def update_personal():
    ...
    try:
        return (
            jsonify(PersonalModel.update_personal(current_personal["id"], data)),
            HTTPStatus.OK,
        )
    except IDNotExistent:
        return {"msg": "Id não encontrado"}, HTTPStatus.NOT_FOUND
    except KeyError:
        return {"msg": "A(s) chave(s) não foi encontrada(s)"}, HTTPStatus.NOT_FOUND
...
def retrieve_personal_id():
    ...
    try:
        return (
            jsonify(PersonalModel.select_by_id(current_personal["id"])),
            HTTPStatus.OK,
        )
    except IDNotExistent:
        return {"msg": "Id não encontrado"}, HTTPStatus.NOT_FOUND
