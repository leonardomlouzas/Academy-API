...
class ExecucaoModel(db.Model):
    ...
    exercicio_id = db.Column(
        db.Integer, db.ForeignKey("exercicio.id"), nullable=False, unique=True
    )

    exercicio = db.relationship(
        "ExercicioModel", back_populates="execucao", uselist=False
    )
    ...
    def validate(self, key, value):
        ...
        if type(value) != int and key in ["series", "repeticoes"]:
            raise TypeNotAccepted("Séries e repetições devem ser do tipo inteiro")
        return value
